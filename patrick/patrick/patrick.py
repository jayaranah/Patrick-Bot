...
def helpmessage():
    if settings['setKey'] == True:
        ...
        key = ''
    helpMessage =   "╔══[ Help Message Bikini Bottom ]" + "\n" + \
                    "╠ " + key + "Help" + "\n" + \
                    "╠ " + key + "Translate" + "\n" + \
                    "╠ " + key + "TTS" + "\n" + \
                    "╠══[ Status Command ]" + "\n" + \
                    ...
                    "╚══[ ~~Bot patrick~~ edited by  https://line.me/ti/p/54f0586zGF ]"
    return helpMessage
...
def clientBot(op):
    try:
        ...
        if op.type == 5:
            ...
            sendMention(op.param1, "Halo @!,terimakasih telah menambahkan patrick sebagai teman :3")
        ...
        if op.type == 13:
            ...
            if clientMid in op.param3:
                ...
                sendMention(op.param1, "Halo @!, Terimakasih Telah Mengundang patrick:3")
                    ...
                    if msg.contentType == 0:
                        if text is None:
                            ...
                            if cmd == "help":
                                ...
                                start = time.time()
                                client.sendMessage(to, "menghitung...")
                                ...
                                client.sendMessage(to, "Patrick sudah berjalan selama {}".format(str(runtime)))
                            elif cmd == "restart":
                                client.sendMessage(to, "Berhasil merestart Patrick")
                                restartBot()
# Pembatas Script #
                            elif cmd == "autoadd on":
...
# Pembatas Script #
                                ...
                                if receiver in read['readPoint']:
                                    ...
                                    read['ROM'][receiver] = {}
                                    client.sendMessage(receiver,"Lurking wes diaktifno hahaha")
                                else:
                                    try:
                                        del read['readPoint'][receiver]
                                        del read['readMember'][receiver]
                                    ...
                                    read['readTime'][receiver] = readTime
                                    read['ROM'][receiver] = {}
                                    client.sendMessage(receiver,"lurking diatur : \n" + readTime)
                            ...
                            for mention in mentionees:
                                if clientMid in mention["M"]:
                                    if settings["autoRespon"] == True:
                                        sendMention(sender, "hei @!,jangan main tag tag dong (petrick) ", [sender])
                                    break
            except Exception as error:
                logError(error)
                traceback.print_tb(error.__traceback__)
